...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean passwordIsValid(Context context, String pass) {
        if(!pass.equals("")){
            if(pass.length() >= 5){
                ...
                for (int i = 0; i < pass.length(); i++) {
                    if (specialCharacters.contains(String.valueOf(pass.charAt(i)))) {
                        ...
                        Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Error6");
                        return false;
                    }
                }
                return true;
                ...
                Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Error7");
            ...
            Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Error8");
    ...
    public boolean loginUserWithSharedPreferences(Context context, ServiceUser serviceUser){
            ...
            if (resultSet.moveToFirst()){
                ...
                if(columnPass != null){
                    if(serviceUser.getPass().equals(columnPass)){
                        ...
                        Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), id.toString());
        ...
        Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Error5");
        return false;
    }
    public boolean loginUser(Context context, ServiceUser serviceUser) {
        if(validateUserData(context, serviceUser, false)){
            ...
            if (resultSet.moveToFirst()){
                ...
                if(columnPass != null){
                    if(Hasher.checkPassword(serviceUser.getPass(), columnPass)){
                        ...
                        if(Util.putUserSharedPreference(context, serviceUser)){
                            resultSet.close();
                            sqLiteDatabase.close();
                            Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Login successful, put sp successful");
                            return true;
                        }
                        else{
                            resultSet.close();
                            sqLiteDatabase.close();
                            Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Error1");
                            return false;
                        }
                    }
                    else{
                        ...
                        resultSet.close();
                        sqLiteDatabase.close();
                        Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Error2");
                        return false;
                    }
                }
                else{
                    ...
                    resultSet.close();
                    sqLiteDatabase.close();
                    Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Error3");
                    return false;
                }
            }
            else{
                ...
                resultSet.close();
                sqLiteDatabase.close();
                Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Error4");
                return false;
            }
        }
        Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Error5");
    ...
    public boolean validateUserData(Context context, ServiceUser serviceUser, boolean usingSharedPreferences){
        Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "in data validation" + serviceUser.toString());
        if(serviceUser.getEmail().equals("")){
            Toast.makeText(context, "Email cannot be empty", Toast.LENGTH_SHORT).show();
            Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "data validation email empty");
            return false;
        }
        if(serviceUser.getUsername().equals("")){
            Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "data validation username empty" + serviceUser.toString());
    ...
    public boolean changeUserPassword(Context context, String oldPass, String newPass, String confirmNewPass){
        ...
        if(serviceUser != null){
            ...
            if(checkPasswordValidityWhenChanging(context, id, oldPass, newPass, confirmNewPass)){
                ...
                Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), "Rows: " + rows);
                if(rows > 0){
                    Toast.makeText(context, "Password successfully updated", Toast.LENGTH_SHORT).show();
                    sqLiteDatabase.close();
                    serviceUser.setPass(hashedPass);
                    Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), serviceUser.toString());
                    if(Util.putUserSharedPreference(context, serviceUser)){
                        Log.i(context.getResources().getString(rs.raf.projekat1.lazar_bojanic_rn11621.R.string.dnevnjakTag), Util.getUserSharedPreference(context).toString());
                        return true;
                    }
                    else{
                        return false;
